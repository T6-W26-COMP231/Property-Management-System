...
import SearchContractorPage    from "./SearchContractorPage";
...
export default function MaintenanceRequestsPage({ property, onBack }) {
  ...
  const handleUnassign = async (request) => {
    try {
      const token = await getAccessTokenSilently();
      await unassignContractor(token, request._id);
      await load();
      showToast("success", "Contractor removed. Request is now Unassigned.");
    } catch {
      showToast("danger", "Failed to remove contractor.");
    }
  };

  // ── Pagination — only show Submitted and In Progress ─────────────────────
  const activeRequests = requests.filter((r) => r.status !== "Completed");
  const totalPages     = Math.ceil(activeRequests.length / PER_PAGE);
  const paginated      = activeRequests.slice((page - 1) * PER_PAGE, page * PER_PAGE);

  // Navigate to SearchContractorPage
  if (searchRequest) {
    return (
      <SearchContractorPage
        request={searchRequest}
        onBack={() => setSearchRequest(null)}
        onAssigned={() => { setSearchRequest(null); load(); }}
      />
    );
  }
  ...
  return (
    <div className="p-4">
      ...
      <MaintenanceViewModal
        ...
        onSearchContractor={(r) => setSearchRequest(r)}
